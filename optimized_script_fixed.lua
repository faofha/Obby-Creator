...
-- Declared here (not down by the action functions) because makeRow's rebind
-- closure below writes into this table on every keybind change — it needs to
-- already exist by then. Populated with real actions further down once
-- pauseFunc/createFunc/deleteFunc/teleportFunc exist.
local keyActions = {}
...
-- ══════════════════════════════════════════════════════════════════════════════
-- CHECKPOINT PART FACTORY + FLAT SAVE/LOAD
-- createCheckpointPart is a direct port from the pre-optimization script.
-- saveCheckpoints/loadCheckpoints are new — the earlier script had no file
-- persistence at all, but SAVE_FILE/serializeCP/encodeVal/jsonDecode above were
-- already built and unused, and loadSlot further below is an exact template for
-- this minus the per-obby origin remap, so this is a low-risk fill-in rather
...
local function saveCheckpoints()
    if cpCounter <= 0 then return false, "No checkpoints to save" end
    ensureDir()
    ...
    fileWrite(SAVE_FILE, encodeVal(data))
    return true, "Saved " .. #data.checkpoints .. " checkpoint(s)"
end

local function loadCheckpoints()
    local raw = fileRead(SAVE_FILE)
    ...
    for _, entry in ipairs(data.checkpoints) do
        ...
        cp.HumanoidStateType.Value                = entry.state or "Landed"
        cp.Parent = CPFolder
    end
    cpCounter = tonumber(data.counter) or cpCounter
    return true, "Loaded " .. cpCounter .. " checkpoint(s)"
end
...
local function makeActionButton(id, label, fn)
    ...
    if type(fn) == "function" then
        ...
        -- fn is missing (nil) — connect a harmless stand-in instead of letting
        -- Connect() throw "Passed value is not a function" at script load.
        Btn.MouseButton1Click:Connect(function()
            warn("[AdvancedCP] '"..label.."' has no handler defined — this button is a no-op until that's fixed.")
        end)
    end
    return Btn
end
...
-- ── Obby parsing helper (hoisted above its callers below — it used to only
-- exist ~330 lines further down inside `if game.PlaceId == 2913303231`, so
-- every call up here was resolving to a nil global and erroring) ────────────
...
-- ══════════════════════════════════════════════════════════════════════════════
-- CORE ACTIONS  (restored from the pre-optimization script — these and the
-- keyActions dispatch table were dropped entirely during "optimization";
-- ported here essentially unchanged)
-- ══════════════════════════════════════════════════════════════════════════════
...
local function teleportFunc()
    -- tpEnabled guard added during integration: the "Teleport Enabled" toggle
    -- above already existed in the UI but wasn't wired to anything. Defaults to
    -- true, so this is a no-op unless the toggle is actually switched off.
...
-- Populate the dispatch table declared near the top of the script (keyActions
-- was already there as an empty table so the rebind closures had something
-- safe to write into before these functions existed).
keyActions[pauseKey]    = pauseFunc
keyActions[createKey]   = createFunc
keyActions[deleteKey]   = deleteFunc
keyActions[teleportKey] = teleportFunc
keyActions[menuKey]     = function() MenuFrame.Visible = not MenuFrame.Visible end
...
-- ── Camera tilt (PageUp/PageDown) + keybind dispatch ─────────────────────────
-- Restored along with the actions above — the optimized file built keyActions
-- entries on rebind but never had anything that actually READ keyActions on a
-- keypress, so E/F/V/R/M were dead even before pauseFunc etc. went missing.
...
local function rotateCameraVertical(dir)
    ...
    cam.CFrame = CFrame.new(pos) * CFrame.Angles(0, yaw, 0) * CFrame.Angles(camTilt, 0, 0)
end
...
local _topbarOk, _topbarErr = pcall(function()
    ...
    local iconsFolder = RS_mod:WaitForChild("Icons", 10)
    if not iconsFolder then error("ReplicatedStorage.Icons never appeared") end
    local topBarIconModule = iconsFolder:WaitForChild("TopBarIcon", 10)
    if not topBarIconModule then error("Icons.TopBarIcon never appeared") end
    local Icon = require(topBarIconModule)
